...
const messageurl: string = "https://eddy-twitter-api.vercel.app/api/v1/message";
  ...
  async (messageData, { rejectWithValue, getState }) => {
    try {
      ...
      const response2 = await axios.post(
        `https://eddy-twitter-api.vercel.app/api/v1/message/${messageData?.conversationId}`,
        messageData,
        config
      );
...
export const Deletemessage = createAsyncThunk<string, { Detailsdata: string }, {
  ...
  async ({ Detailsdata }, { rejectWithValue, getState }) => {
    ...
    try {
      ...
      await axios.delete(
        `https://eddy-twitter-api.vercel.app/api/v1/message/${Detailsdata}`,
        config
      );
      return Detailsdata;
...
export const GetUsermessage = createAsyncThunk<{
  ...
  async (Detailsdata, { rejectWithValue, getState }) => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://eddy-twitter-api.vercel.app/api/v1/message/user/${Detailsdata}`,
        config
      );
      return response.data.message;
